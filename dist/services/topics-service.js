...
exports.analyzeTopics = analyzeTopics;
...
for (const w of wordcloud_service_1.SPANISH_STOPWORDS)
    ALL_STOPWORDS.add(w);
for (const w of wordcloud_service_1.ENGLISH_STOPWORDS)
    ALL_STOPWORDS.add(w);
...
for (const w of EXTRA_STOPWORDS)
    ALL_STOPWORDS.add(w);
function classifySentiment(text) {
    ...
    for (const w of words) {
        ...
        if (sentiment_service_1.POSITIVE_WORDS.has(clean))
            pos++;
        if (sentiment_service_1.NEGATIVE_WORDS.has(clean))
            neg++;
    }
    if (pos > neg)
        return 'positivo';
...
function analyzeTopics(data) {
    ...
    for (const term of topTerms) {
        const termMsgs = new Set(msgTerms.filter(mt => mt.terms.has(term)).map(mt => mt.msg));
        if (termMsgs.size < 2)
            continue;
        let bestIdx = -1;
        let bestOverlap = 0;
        ...
        if (bestIdx >= 0) {
            for (const m of termMsgs)
                clusters[bestIdx].messages.add(m);
