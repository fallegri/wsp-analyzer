...
export function generateWordCloud(data: ConversationData, limit: number = 100): WordCloudEntry[] {
  ...
  for (const m of messages) {
    ...
    for (const w of words) {
      ...
      if (SPANISH_STOPWORDS.has(cleaned)) continue;
      if (ENGLISH_STOPWORDS.has(cleaned)) continue;
